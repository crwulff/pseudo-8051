...
def _fold_xram_call_args(nodes: List[HIRNode]) -> List[HIRNode]:
    ...
    """
    nodes = recurse_bodies(nodes, _fold_xram_call_args)
    ...
    for i in range(len(work)):
        ...
        # Get callee's XRAM params in declared order
        try:
        ...
        collected: Dict[str, tuple] = {}  # param_name → (node_index, rhs_expr)
        j = i - 1
        while j >= 0:
            if j in removed:
                j -= 1
        ...
        # All params found — build extra args in declared order and patch the call
